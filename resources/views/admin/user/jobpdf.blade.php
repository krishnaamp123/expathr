...
<body>
    <div class="profile-container">
        <div class="profile-row">
            ...
            <div class="profile-col-text">
                ...
                <p class="center">{{ $user->address }} | {{ Carbon::parse($user->birth_date)->format('d-m-Y') }} | {{ $user->gender }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="other-row">
            <div class="other-col-text1">
                ...
                @foreach ($user->education as $edu)
                    ...
                    <p class="tight">
                        {{ Carbon::parse($edu->start_date)->format('m-Y') }} -
                        {{ $edu->end_date === null ? 'Present' : Carbon::parse($edu->end_date)->format('m-Y') }}
                    </p>
                @endforeach
            </div>
            ...
            @if ($user->project->whereNotNull('media')->count())
                ...
                <div class="lain-col-text">
                    @foreach ($user->project as $pro)
                        ...
                        <p class="tight">
                            {{ Carbon::parse($pro->start_date)->format('m-Y') }} -
                            {{ $pro->end_date === null ? 'Present' : Carbon::parse($pro->end_date)->format('m-Y') }}
                        </p>
    ...
    @foreach ($user->experience as $exp)
        ...
        <p class="tight">
            ...
            {{ $exp->end_date === null ? 'Present' : Carbon::parse($exp->end_date)->format('m-Y') }}
        </p>
            ...
            @if ($user->certification->whereNotNull('media')->count())
                ...
                <div class="lain-col-text">
                    @foreach ($user->certification as $crt)
                        ...
                        <p class="tight">
                            {{ Carbon::parse($crt->start_date)->format('m-Y') }} -
                            {{ $crt->end_date === null ? 'Present' : Carbon::parse($crt->end_date)->format('m-Y') }}
                        </p>
    ...
    @foreach ($user->organization as $org)
        ...
        <p class="tight">
            {{ Carbon::parse($org->start_date)->format('m-Y') }} -
            {{ $org->end_date === null ? 'Present' : Carbon::parse($org->end_date)->format('m-Y') }}
        </p>
